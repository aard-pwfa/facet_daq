...
% Last Modified by GUIDE v2.5 12-Nov-2009 11:06:13
...
% List of index names.
handles.indexList={'LCLS' {'IN20' 'LI21' 'LI24' 'LI25' 'LI29'}; ...
    'FACET' {'LI20'}; ...
...
%handles.sector.nameList={'IN20' 'LI21' 'LI24' 'LI25' 'LI29'};
...
handles.fdbkList={'FBCK:INL0:1:ENABLE';'FBCK:INL1:1:ENABLE';'FBCK:FB01:TR02:MODE'; ...
...
handles.configList={'measurePhaseValNum' 'processNumBG' 'processAverage' ...
    'processSampleNum' 'processSelectMethod'};
...
str={'Off' 'On'};
set(handles.tcav1Hz_btn,'Visible',str{ismember(handles.sectorSel,{'LI25' 'LI29'})+1}); 
...
% ------------------------------------------------------------------------
function handles = measureTcavSet(hObject, handles, val)
...
if strcmp(handles.accelerator,'FACET'), bpmPV='BPMS:LI19:201:TMIT57';end
...
if strcmp(handles.measureType,'cal') || ismember(handles.measureTcavKlys,handles.acclList)
    fbck=lcaGet(handles.fdbkList,0,'double');
...
plane='y';if ismember(handles.measureTcavKlys,handles.acclList), plane='x';end
...
if ismember(handles.measureTcavKlys,handles.acclList), opts.plane='x';end
if 1
%    opts.unitsT='ps';
%    scl=1e12/2856e6/360;
...
if ~handles.process.done
    for iMethod=1:size(data.beam,2)
        ...
        if strcmp(data.type,'blen')
            try
                [sigx,data.blen(:,iMethod),sigxstd,data.blenStd(:,iMethod),r35,r35std]= ...
...
if ~strcmp(data.type,'blen'), return, end
...
measureTcavSet(hObject,handles,val);
